...
import com.iwhalecloud.bss.magic.magicapi.core.event.EventAction;
...
public class UbaMagicResourceService implements MagicResourceService, JsonCodeConstants, ApplicationListener<ApplicationStartedEvent> {
    ...
    private final Logger logger = LoggerFactory.getLogger(UbaMagicResourceService.class);

    public UbaMagicResourceService(Resource resource, List<MagicResourceStorage<? extends MagicEntity>> storages, ApplicationEventPublisher publisher) {
        this.root = resource;
    ...
    private <T extends MagicEntity> MagicResourceStorage getStorage(T entity){
    ...
    public <T extends MagicEntity> boolean saveFile(T entity) {
        ...
        return writeLock(() -> {
            ...
            MagicResourceStorage storage = getStorage(entity);
            // 检查脚本
            ...
            if (flag) {
                //在这里将entity中占位符转换为实际信息，再写入内存
                T realEntity = (T)storage.read(fileResource.read());//获取存到介质中的真实信息（含占位符），并再storage.read中转换成实际信息
                //将转换好之后的entity存到内存，并发送通知出去
                publisher.publishEvent(new FileEvent(storage.folder(), action, realEntity));
                putFile(storage, realEntity, fileResource);
            }
            return flag;
        });
    }
    ...
    /**根据ID获取文件，如果realContent传入为true，表示不要从缓存获取，而从数据库获取*/
    public <T extends MagicEntity> T file(String id, boolean realContent){
        T entity = (T) fileCache.get(id);
        if(realContent){
            Resource fileResource = fileMappings.get(id);
            MagicContext.setReplaceFlag(false);
            try {
                return (T) getStorage(entity).read(fileResource.read());
            }finally {
                MagicContext.clearReplaceFlag();
            }
        }
        return entity;
    }
